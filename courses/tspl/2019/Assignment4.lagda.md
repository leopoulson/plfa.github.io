...
```
  ...
  ⊢bimul : ∅ ⊢ bimul ↑ `ℕ ⇒ `ℕ ⇒ `ℕ
  ⊢bimul with toWitness { Q = synthesize ∅ bimul } tt
  ... | ⟨ `ℕ ⇒ `ℕ ⇒ `ℕ , pf ⟩ =  pf
  ...
  mul = DB.μ DB.ƛ DB.ƛ DB.case (DB.# 1)
                   (DB.`zero)
                   (dbplus DB.· (DB.# 1) DB.· (DB.# 3 DB.· DB.# 0 DB.· DB.# 1))
    where open DeBruijn
...
```
  ...
  eval2 : ∀ {Γ A}
    ...
    → (L : Γ ⊢ A)
      -----------
    → Steps L
  eval2 (gas zero)    L                     =  steps (L ∎) out-of-gas
  eval2 (gas (suc m)) L with progress L
  ... | done NrmL                          =  steps (L ∎) (done NrmL)
  ... | step {M} L—→M with eval2 (gas m) M
  ...    | steps M—↠N fin                  =  steps (L —→⟨ L—→M ⟩ M—↠N) fin
